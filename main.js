...
class BasicCharacterController {
  ...
  _LoadModels() {
    ...
    loader.load('swatg.fbx', (fbx) => {
      ...
      loader.load('backwardw.fbx', (a) => { _OnLoad('backward', a); });
  ...
  _UpdateCamera() {
    const offset = new THREE.Vector3(0, 15, -45);
...
class CharacterFSM extends FiniteStateMachine {
  ...
  _Init() {
    ...
    this._AddState('backward', BackwState);
...
class BackwState extends State {
  constructor(parent) {
    super(parent);
  }
...
class RunState extends State {
  ...
  Enter(prevState) {
    const curAction1 = this._parent._proxy._animations['run'].action;
    if (prevState) {
      const prevAction = this._parent._proxy._animations[prevState.Name].action;

      curAction1.enabled = true;

      if (prevState.Name == 'walk') {
        const ratio = curAction1.getClip().duration / prevAction.getClip().duration;
        curAction1.time = prevAction.time * ratio;
      } else {
        curAction1.time = 0.0;
        curAction1.setEffectiveTimeScale(1.0);
        curAction1.setEffectiveWeight(1.0);
      }

      curAction1.crossFadeFrom(prevAction, 0.5, true);
      curAction1.play();
    } else {
      curAction1.play();
    }
...
class CharacterControllerDemo {
  ...
  _Initialize() {
    ...
    this._camera.position.set(5, 100, 100);
    ...
    this._scene.fog = new THREE.Fog(0x595552, 50, 100);  // Color, near, far
    ...
    const groundMaterial = new THREE.MeshStandardMaterial({ color: 0x5c5151 });
    ...
    const cubeGeometry = new THREE.BoxGeometry(10, 10, 2);
    const cubeMaterial = new THREE.MeshStandardMaterial({ color: 0xd111a7 });
    this._collisionCube = new THREE.Mesh(cubeGeometry, cubeMaterial);

    this._collisionCube.position.set(1, 5, 3); // Ajustar la posición del cubo
  ...
  _InitializeGUI() {
    const walkButton = document.getElementById('btnWalk');
    const runButton = document.getElementById('btnRun');
    const idleButton = document.getElementById('btnIdle');

    walkButton.addEventListener('click', () => {
      if (this._controls) {
        this._controls.SetState('walk');
      }
    });

    runButton.addEventListener('click', () => {
      if (this._controls) {
        this._controls.SetState('run');
      }
    });
  ...
  generateRectanges(scene) {
    const brownMaterial = new THREE.MeshStandardMaterial({ color: 0xd66f38 }); // Color café (marrón) para los cubos
    ...
    for (let i = 0; i < numCubes; i++) {
      ...
      // Crear el cubo
      const cubeGeometry = new THREE.BoxGeometry(30, 30, 30); // Dimensiones del cubo
      ...
      // Crear el cubo
      const cubeGeometry1 = new THREE.BoxGeometry(100, 10, 10); // Dimensiones del cubo
